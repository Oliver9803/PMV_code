...
if (example==11){
delta=seq(0, 0.6, by=.1)
}

if (example==12){
delta=seq(0, 9, by=1.5)
}

if (example==21){
delta=seq(1, 4.5, by=.5)
}
...
em.power=array(0,dim=c(4, 3, length(delta)))
...
fit<-manova(X~Y)
dcortpVal[ii,1]=summary(fit, test="Wilks")$stats[1, "Pr(>F)"]
...
matplot(delta, t(em.power[,2,]),type="b",lty = 2:5, xlab=expression(delta),ylab="empirical power",
lwd=3, pch = 21:24, col = c(1,2,4,6) )
