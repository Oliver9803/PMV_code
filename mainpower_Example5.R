...
dis2<- disco(X, factors=Y, index=0.8, R=BstrpTimes)
discpva2[ii,1]=as.numeric(dis2$p.value) 
     ...
     cat("disco_0.8= ", discpva2.mean,"\n")

 
 
dis3<-disco(X, factors=Y, index=0.5, R=BstrpTimes)
discpva3[ii,1]=as.numeric(dis3$p.value) 
     ...
     cat("disco_0.5= ", discpva3.mean,"\n")


dis4<-disco(X, factors=Y, index=0.2, R=BstrpTimes)
...
matplot(delta, t(em.power[,1,]),type="b",lty = 2:5, xlab=expression(delta),ylab="empirical power",
lwd=3, pch = 21:25, col =col = c(1,2,4,6,5) )

abline(h = 0.01,  col = "lightgray") 
axis(4,  at=0.01,  col.axis = "red", lwd =2)
legend("topleft", c("PMV", "DISCO_1","DISCO_0.8","DISCO_0.5","DISCO_0.2"), lty = 2:6,lwd=2,
cex = 0.85, pch = 21:25, col = c(1,2,4,6,5) )




########## type I error at 0.05
matplot(delta, t(em.power[,2,]),type="b",lty = 2:6, xlab=expression(delta),ylab="empirical power",
lwd=3, pch = 21:25, col = c(1,2,4,6,5) )
...
matplot(delta, t(em.power[,3,]),type="b",lty = 2:5, xlab=expression(delta),ylab="empirical power",
lwd=3, pch = 21:25, col = c(1,2,4,6,7) )
